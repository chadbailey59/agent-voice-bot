...
@dataclass(frozen=True)
class AgentLoopRunHandle:
    """A cancellable backend run."""
...
class AgentLoopEvent:
    """Normalized event from a backend run."""
...
@dataclass(frozen=True)
class AgentLoopFollowupResult:
    """Outcome of sending a follow-up to an active backend run."""
...
class AgentLoopClient:
    ...
    async def events(self, handle: AgentLoopRunHandle) -> AsyncIterator[AgentLoopEvent]:
        ...
        if handle.backend == "legacy":
            ...
            yield AgentLoopEvent("final", text=result.summary, raw=result.raw)
            return
            ...
            async with client.stream("GET", url, headers=_hermes_headers(self._config)) as response:
                ...
                async for payload in _iter_sse_json(response):
                    ...
                    if event_name == "message.delta":
                        yield AgentLoopEvent("delta", text=str(payload.get("delta", "")), raw=payload)
                    elif event_name == "run.completed":
                        yield AgentLoopEvent("final", text=str(payload.get("output", "")), raw=payload)
                        return
                    elif event_name == "run.failed":
                        yield AgentLoopEvent("error", text=str(payload.get("error", "")), raw=payload)
                        return
                    elif event_name in {"run.cancelled", "run.stopped"}:
                        yield AgentLoopEvent("aborted", raw=payload)
                        return
                    elif event_name:
                        yield AgentLoopEvent("progress", text=event_name, raw=payload)
    ...
    async def _events_openclaw(self, handle: AgentLoopRunHandle) -> AsyncIterator[AgentLoopEvent]:
        ...
        try:
            while True:
                ...
                if state == "delta":
                    yield AgentLoopEvent("delta", text=text, raw=payload)
                elif state == "final":
                    yield AgentLoopEvent("final", text=text, raw=payload)
                    return
                elif state == "aborted":
                    yield AgentLoopEvent("aborted", text=text, raw=payload)
                    return
                elif state == "error":
